...
void igvInterfaz::set_glutKeyboardFunc(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case 'y':
            if (interfaz.escena.getInstanciasYsceneB() > MIN_INSTANCIAS) {
                interfaz.escena.setInstanciasYsceneB(interfaz.escena.getInstanciasYsceneB() - 1);
            }
            if (interfaz.escena.getInstanciasY() > 0) {
                interfaz.escena.setInstanciasY(interfaz.escena.getInstanciasY() - 1);
            }

            break;

        case 'Y':
            if (interfaz.escena.getInstanciasYsceneB() < MAX_INSTANCIAS) {
                interfaz.escena.setInstanciasYsceneB(interfaz.escena.getInstanciasYsceneB() + 1);
            }
            if (interfaz.escena.getInstanciasY() < MAX_INSTANCIAS) {
                interfaz.escena.setInstanciasY(interfaz.escena.getInstanciasY() + 1);
            }
            //interfaz.escena.visualizar(interfaz.escena.EscenaA); //TODO
            break;

        case 'x':
            if (interfaz.escena.getInstanciasX() > MIN_INSTANCIAS) {
                interfaz.escena.setInstanciasX(interfaz.escena.getInstanciasX() - 1);
            }
            break;

        case 'X':
            if (interfaz.escena.getInstanciasX() < MAX_INSTANCIAS) {
                interfaz.escena.setInstanciasX(interfaz.escena.getInstanciasX() + 1);
            }
            break;

        case 'z':
            if (interfaz.escena.getInstanciasZ() > MIN_INSTANCIAS) {
                interfaz.escena.setInstanciasZ(interfaz.escena.getInstanciasZ() - 1);
            }
            break;

        case 'Z':
            if (interfaz.escena.getInstanciasZ() < MAX_INSTANCIAS) {
                interfaz.escena.setInstanciasZ(interfaz.escena.getInstanciasZ() + 1);
            }
            break;
